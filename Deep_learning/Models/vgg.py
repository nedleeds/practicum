...
class vgg():
    ...
    def __call__(self, input_imgs):
        ...
        model.add(layers.Dense(4096, activation='relu', kernel_regularizer=regularizers.l2(1e-3)))
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(2048, activation='relu', kernel_regularizer=regularizers.l2(1e-3)))
        model.add(layers.Dropout(0.1))
        model.add(layers.Dense(1024, activation='relu', kernel_regularizer=regularizers.l2(1e-3)))
        # model.add(layers.Dropout(0.1))

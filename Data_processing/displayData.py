    ...
    def __call__(self, idx=0, numToShow=1, colormap="gray", opt='default'): #idx should be int or list, opt:default-> 'nothing'
        ...
        if self.len==1 : 
            if self.idx==False:
                ...
                idcs = keys[self.idx]
                plt.figure(figsize=(15,15))
                if self.opt.lower() =='hist' : plt.hist(self.images[idcs]), plt.title(f'octa img[{idcs}]')
                else : plt.imshow(self.images[idcs], cmap='gray'), plt.title(f'octa img[{idcs}]'), plt.axis(False)
